...
def read_match_data(file_path):
    ...
    for batter in india_innings.get("batting", []):
        if "dismissal" in batter:
            print(
                f"  {batter['batsman']:<20} | {batter['runs']:<3} ({batter['balls']}) | 4s: {batter['fours']} | 6s: {batter['sixes']} | SR: {batter['strike_rate']} | {batter['dismissal']}")
        else:
    ...
    for bowler in india_innings.get("bowling", []):
        print(
            f"  {bowler['bowler']:<18} | {bowler['overs']} overs | {bowler['maidens']} maidens | {bowler['runs']} runs | {bowler['wickets']} wickets | Econ: {bowler['economy']}")
